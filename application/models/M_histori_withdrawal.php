...
class M_histori_withdrawal extends CI_Model
{
    ...
    function m_get_datasales($id_sales)
    {
        $this->db->select('sales.nama, sales.email, sales.no_wa');
        $this->db->from('sales');
    ...
    function get_rekening_withdrawal($no_wd)
    {
        $this->db->select('rekening_an, bank_nm, rekening_no');
        $this->db->from('transaksi_sales');
        $this->db->where('no_wd', $no_wd);
        $query = $this->db->get();
        return $query->result();
    }
}
